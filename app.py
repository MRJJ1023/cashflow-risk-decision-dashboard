...
focus_df = filtered[filtered["risk_level"].eq("重点关注") | (filtered["rule_hit_count"] > 0)].copy()
...
<div class="hero">
  ...
  <div class="subtitle">支出结构 × 预算偏差 × 风险规则识别 × 落地行动清单｜脱敏数据作品集 v01</div>
  <div class="judgement">
    <b>关键判断：</b> 当前筛选范围内，日常经营支出主要集中在 <b>{top_cat}</b>，重点关注交易 <b>{focus_count}</b> 笔；
...
metrics = [
    ...
    ("重点关注交易", f"{focus_count} 笔", "规则命中或重点关注"),
...
with c6:
    ...
    fig_scene = px.scatter(scene_diag, x="spend", y="focus", size="count", color="预算外金额", hover_name="scene_l2", text="scene_l2", title="场景金额 × 重点关注矩阵", color_continuous_scale="OrRd")
    ...
    fig_scene.update_traces(textposition="top center", marker=dict(sizemin=8, opacity=.78))
...
focus_display = filtered[filtered["rule_hit_count"] > 0].copy().sort_values("action_priority_score", ascending=False)
if focus_display.empty:
    st.info("当前筛选范围暂无规则命中交易。")
else:
